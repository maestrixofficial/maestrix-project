...
import { userRecordChart } from '@/utils/chartsLogic/usersRecordChart';

const PaymentRecordChart = () => {
    // const chartOptions = paymentRecordChartOption()
    const chartOptions = userRecordChart()
        ...
        <div className="col-xxl-8">
            <div className={`card stretch stretch-full ${isExpanded ? "card-expand" : ""} ${refreshKey ? "card-loading" : ""}`}>
                <CardHeader
                    ...
                    title={"Users Record"}
                    refresh={handleRefresh} remove={handleDelete} expanded={handleExpand} />
                ...
                <div className="card-footer">
                    ...
                    <div className="row g-4">
                        <Card count={"486"} title={"Total Users"} />
                        <Card count={"275"}  title={"Currently Active"} />
                        <Card count={"168"}  title={"Total Subscribed"} />
                        <Card count={"68"}  title={"Unactive"} />
                    </div>
                </div>
...
const Card = ({ title, count }) => {
    return (
        <div className="col-lg-3">
            <div className="p-3 border border-dashed rounded">
                <div className="fs-12 text-muted mb-1">{title}</div>
                <h6 className="fw-bold text-dark">{count}</h6>
                {/* <div className="progress mt-2 ht-3">
                    <div className={`progress-bar ${bg_color}`} role="progressbar" style={{ width: progress }}></div>
                </div> */}
            </div>
